...
/**
 * @fileoverview Number angle input field.
 */
...
/**
 * Options used to define a field angle from JSON.
 */
export interface FieldAngleOptions extends FieldAngleConfig {
  value?: string | number;
}

export type FieldAngleValidator = FieldNumberValidator;
...
export class FieldAngle extends Blockly.FieldNumber {
  ...
  static readonly HALF_ = 100 / 2;

  /**
   * Radius of protractor circle.  Slightly smaller than protractor size since
   * otherwise SVG crops off half the border at the edges.
   */
  static readonly RADIUS_: number = FieldAngle.HALF_ - 1;
  ...
  /**
   * Class for an number angle field.
  ...
  constructor(value?: string | number,
    ...
    super(value, undefined, undefined, undefined, validator, config);
    if (config) {
  ...
  /**
   * Constructs a FieldAngle from a JSON arg object.
  ...
  override initView() {
    ...
    this.symbol = Blockly.utils.dom.createSvgElement(Blockly.utils.Svg.TSPAN, {});
   ...
   *
   * @param opt_e Optional mouse event that triggered the field to open,
   ...
   */
  private dropdownCreate_(): SVGSVGElement {
    const svg = Blockly.utils.dom.createSvgElement(Blockly.utils.Svg.SVG, {
      'xmlns': Blockly.utils.dom.SVG_NS,
      'xmlns:html': Blockly.utils.dom.HTML_NS,
      'xmlns:xlink': Blockly.utils.dom.XLINK_NS,
      'version': '1.1',
      'height': FieldAngle.HALF_ * 2 + 'px',
      'width': FieldAngle.HALF_ * 2 + 'px',
    ...
    for (let angle = 0; angle < 360; angle += 15) {
      Blockly.utils.dom.createSvgElement(
          Blockly.utils.Svg.LINE, {
            'x1': FieldAngle.HALF_ + FieldAngle.RADIUS_,
            'y1': FieldAngle.HALF_,
            'x2': FieldAngle.HALF_ + FieldAngle.RADIUS_ -
                (angle % 45 === 0 ? 10 : 5),
            'y2': FieldAngle.HALF_,
            'class': 'blocklyAngleMarks',
            'transform': 'rotate(' + angle + ',' + FieldAngle.HALF_ + ',' +
                FieldAngle.HALF_ + ')',
          }, svg);
    }
  ...
  protected onMouseMove_(e: PointerEvent) {
    ...
    const dx = e.clientX - bBox.left - FieldAngle.HALF_;
    const dy = e.clientY - bBox.top - FieldAngle.HALF_;
  ...
  private updateGraph_() {
    ...
    const path = ['M ', FieldAngle.HALF_, ',', FieldAngle.HALF_];
    let x2 = FieldAngle.HALF_;
    let y2 = FieldAngle.HALF_;
    if (!isNaN(angleRadians)) {
      const clockwiseFlag = Number(this.clockwise);
      const angle1 = Blockly.utils.math.toRadians(this.offset);
      const x1 = Math.cos(angle1) * FieldAngle.RADIUS_;
      const y1 = Math.sin(angle1) * -FieldAngle.RADIUS_;
      if (clockwiseFlag) {
        angleRadians = 2 * angle1 - angleRadians;
      }
      x2 += Math.cos(angleRadians) * FieldAngle.RADIUS_;
      y2 -= Math.sin(angleRadians) * FieldAngle.RADIUS_;
      ...
      path.push(
          ' l ', x1, ',', y1, ' A ', FieldAngle.RADIUS_, ',', FieldAngle.RADIUS_,
  ...
  protected override onHtmlInputKeyDown_(e: KeyboardEvent) {
    ...
    if (multiplier) {
      ...
      this.displayMouseOrKeyboardValue_(value + multiplier * this.round);
      e.preventDefault();
      e.stopPropagation();
    }
  }
 ...
 */
export interface FieldAngleConfig extends FieldNumberConfig {
  mode?: Mode;
  clockwise?: boolean;
  offset?: number;
  wrap?: number;
