...
- `dist/` contains the packaged output (that you could host on a server, for example). This is ignored by git and will only appear after you run `npm run build` or `start`.

### Source Code

- `index.html` contains the skeleton HTML for the page. This is modified during the build to inject the JS bundle into the page.
...
- `generators/javascript.js` contains the JavaScript generator for the custom text block. You'll need to include block generators for any custom blocks you create, in whatever language(s) your application will use.
